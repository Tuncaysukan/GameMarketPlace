...
export default class {
    ...
    update(category) {
        ...
        // Update color fields
        $('input[name="background_color"]').val(category.background_color || '#ffffff');
        $('input[name="hover_background_color"]').val(category.hover_background_color || '#f8f9fa');
        $('input[name="text_color"]').val(category.text_color || '#333333');
        $('input[name="hover_text_color"]').val(category.hover_text_color || '#ffffff');
        ...
        $('.category-image .image-holder-wrapper').html(this.categoryImage('category_image', category.category_image));

        $('#category-form input[name="parent_id"]').remove();
    }

    categoryImage(fieldName, file) {
        if (!file || !file.exists || !file.id || !file.path) {
    ...
    clear() {
        ...
        // Clear color fields
        $('input[name="background_color"]').val('#ffffff');
        $('input[name="hover_background_color"]').val('#f8f9fa');
        $('input[name="text_color"]').val('#333333');
        $('input[name="hover_text_color"]').val('#ffffff');

        $('.logo .image-holder-wrapper').html(this.imagePlaceholder());
        $('.banner .image-holder-wrapper').html(this.imagePlaceholder());
        $('.category-image .image-holder-wrapper').html(this.imagePlaceholder());
