...
class OrderService
{
    public function create($request)
    {
        ...
        // Sipariş oluşturulmadan önce stok kontrolü yap
        $this->checkStockAvailability();

        try {
            $order = $this->store($request);

            // Sipariş başarıyla oluşturulduktan sonra diğer işlemleri yap
            ...
            // Not: Stok azaltma işlemi OrderPlaced event'i tetiklendiğinde ClearCart listener'ında yapılıyor

            return $order;
        } catch (\Exception $e) {
            // Hata durumunda stok geri yükle (eğer azaltılmışsa)
            $this->restoreStock();
            throw $e;
        }
    }
    ...
    public function checkStockAvailability()
    {
        Cart::items()->each(function ($cartItem) {
            if ($cartItem->item->manage_stock) {
                $availableQty = $cartItem->item->qty;
                $requestedQty = $cartItem->qty;

                if ($availableQty < $requestedQty) {
                    throw new \Exception("Ürün '{$cartItem->product->name}' için yeterli stok bulunmamaktadır. Mevcut: {$availableQty}, İstenen: {$requestedQty}");
                }
            }
        });
    }
