...
class CheckoutController extends Controller
{
    ...
    public function store(StoreOrderRequest $request, CustomerService $customerService, OrderService $orderService)
    {
        try {
            ...
            // Sipariş oluştur (stok kontrolü ve hata yönetimi OrderService içinde)
            ...
            try {
                ...
                // Ödeme başarısız olursa siparişi sil ve stok geri yükle
                $orderService->delete($order);

                return response()->json([
                    'message' => 'Ödeme işlemi başarısız oldu: ' . $e->getMessage(),
                ], 403);
            }

        } catch (Exception $e) {
            // Sipariş oluşturma sırasında hata (stok yetersizliği vb.)
            return response()->json([
    ...
    public function create(): View|Factory|Application
    {
        ...
        $gateways = $gateways->map(function ($gateway, $key) {
            if ($key === 'balance') {
                $account = auth()->user();
                $gateway->description = str_replace("(balance)", number_format($account->balance, 2). " TL", $gateway->description);
            }
            return $gateway;
        });
