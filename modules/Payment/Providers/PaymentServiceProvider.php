...
use Modules\Payment\Gateways\Balance;
...
class PaymentServiceProvider extends ServiceProvider
{
    ...
    private function registerPay2out()
    {

        if ($this->enabled('pay2out')) {
            Gateway::register('pay2out', new Pay2out());
        }
    }

    private function registerBalance()
    {

        if ($this->enabled('balance')) {
            Gateway::register('balance', new Balance());
        }
    }

    private function registerShopier()
    {

        if ($this->enabled('shopier')) {
            Gateway::register('shopier', new Shopier());
        }
    }
