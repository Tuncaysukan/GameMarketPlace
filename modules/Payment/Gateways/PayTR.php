...
class PayTR implements GatewayInterface
{
    ...
    public function purchase(Order $order, Request $request)
    {
        ...
        if($result['status']=='success') {
            ...
            // PayTR token başarılı alındı, sipariş durumunu completed yap
            \Log::info('PayTR - Sipariş güncellemesi başlıyor: ' . $order_id . ' - Mevcut durum: ' . $this->order->status);

            $updateResult = $this->order->update(['status' => \Modules\Order\Entities\Order::COMPLETED]);

            // Güncelleme sonucunu kontrol et
            $this->order->refresh(); // Veritabanından tekrar yükle
            \Log::info('PayTR - Güncelleme sonucu: ' . ($updateResult ? 'BAŞARILI' : 'BAŞARISIZ') . ' - Yeni durum: ' . $this->order->status);

            // Sepet temizleme için OrderPlaced event'ini tetikle
            event(new \Modules\Checkout\Events\OrderPlaced($this->order));

            \Log::info('PayTR - Sipariş otomatik completed yapıldı: ' . $order_id . ' - Final durum: ' . $this->order->status);
