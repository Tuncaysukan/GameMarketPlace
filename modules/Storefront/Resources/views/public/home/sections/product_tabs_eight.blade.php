<section x-data="ProductTabsEight({{ $productTabsEight['tabs'] }})" class="landscape-tab-products-wrap">
    <div class="container">
        <div class="landscape-right-tab-products-inner">
            <div class="tab-products-header">
                <h5 class="section-title">{{ $productTabsEight['title'] }}</h5>

                <div class="tab-products-header-overflow">
                    <ul class="tabs">
                        @foreach ($productTabsEight['tabs'] as $key => $tab)
                            <li
                                class="tab-item"
                                :class="classes({{ $key }})"
                                @click="changeTab({{ $key }})"
