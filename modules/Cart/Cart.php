...
class Cart extends DarryldecodeCart implements JsonSerializable
{
    ...
    {
        \Log::info('Cart::clear() çağrıldı - Mevcut item sayısı: ' . $this->getTotalQuantity());

        parent::clear();

        $this->clearCartConditions();

        \Log::info('Cart::clear() tamamlandı - Yeni item sayısı: ' . $this->getTotalQuantity());
    }
    ...
    public function reduceStock()
    {
        $this->manageStock(function ($cartItem) {
            // Debug için log ekle
            \Log::info('Reducing stock for product: ' . $cartItem->product->name . ', Quantity: ' . $cartItem->qty . ', Current Stock: ' . $cartItem->item->qty);

            $cartItem->item->decrement('qty', $cartItem->qty);

            // Azaltma sonrası kontrol
            $cartItem->item->refresh();
            \Log::info('After reduction - New Stock: ' . $cartItem->item->qty);
        });
    }
    ...
    {
        $this->manageStock(function ($cartItem) {
            // Hem product hem de variant için stok geri yükle
            $cartItem->item->increment('qty', $cartItem->qty);
        });
    }
