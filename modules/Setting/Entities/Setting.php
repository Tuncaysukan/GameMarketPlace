...
class Setting extends Model
{
    ...
    public function getValueAttribute()
    {
        ...
        // Güvenli unserialize işlemi
        if (empty($this->plain_value)) {
            return null;
        }

        try {
            $result = unserialize($this->plain_value);

            // Unserialize başarısız olduysa false döner
            if ($result === false && $this->plain_value !== serialize(false)) {
                Log::warning('Setting unserialize failed', [
                    'key' => $this->key,
                    'plain_value' => $this->plain_value,
                    'plain_value_length' => strlen($this->plain_value ?? '')
                ]);
                return null;
            }
            ...
            Log::error('Setting unserialize exception', [
                'key' => $this->key,
                'plain_value' => $this->plain_value,
                'error' => $e->getMessage()
            ]);
