...
class SettingTabs extends Tabs
{
    ...
    private function store()
    {
        return tap(new Tab('store', trans('setting::settings.tabs.store')), function (Tab $tab) {
            ...
            $tab->fields(['translatable.store_name', 'translatable.store_tagline', 'store_phone', 'store_whatsapp', 'store_email', 'store_address_1', 'store_address_2', 'store_city', 'store_country', 'store_state', 'store_zip', 'contact_live_support_text', 'contact_whatsapp_note', 'contact_phone_note', 'contact_help_center_enabled', 'contact_help_center_text', 'contact_help_center_description', 'contact_help_center_url', 'contact_support_request_enabled', 'contact_support_request_text', 'contact_support_request_url', 'legal_info_enabled', 'translatable.legal_provider_title', 'translatable.legal_provider_subtitle', 'legal_provider_info', 'translatable.legal_representative_title', 'translatable.legal_representative_subtitle', 'legal_representative_info', 'translatable.legal_company_title', 'translatable.legal_company_subtitle', 'legal_company_info', 'payment_methods_enabled', 'payment_methods_items']);

            $tab->view('setting::admin.settings.tabs.store', [
                'countries' => Country::all(),
                'media' => $this->getPaymentMethodsMedia(),
            ]);
        });
    }
    ...
    private function getPaymentMethodsMedia()
    {
        $paymentMethods = setting('payment_methods_items', []);
        $media = [];

        // Eğer serialize edilmiş string ise, unserialize et
        if (is_string($paymentMethods)) {
            $paymentMethods = unserialize($paymentMethods);
        }

        // Eğer boş string veya null ise, boş array döndür
        ...
        if (is_array($paymentMethods)) {
            foreach ($paymentMethods as $method) {
                // Boş olan method'ları atla
                if (empty($method['name']) && empty($method['file_id'])) {
                    continue;
                }

                if (!empty($method['file_id'])) {
                    $file = File::find($method['file_id']);
                    if ($file) {
                        $media[$method['file_id']] = $file;
                    }
                }
            }
        }
    ...
    {
        return tap(new Tab('paytr', "PayTR"), function (Tab $tab) {
            $tab->weight(69);
            ...
            $tab->fields(['paytr_enabled', 'translatable.iyzico_label', 'translatable.iyzico_description', 'iyzico_test_mode', 'iyzico_supported_currencies', 'iyzico_api_key', 'iyzico_api_secret']);
    ...
    private function pay2out()
    {
        return tap(new Tab('pay2out', "Pay2Out"), function (Tab $tab) {
            $tab->weight(69);
            ...
            $tab->fields(['pay2out_enabled', 'translatable.iyzico_label', 'translatable.iyzico_description', 'iyzico_test_mode', 'iyzico_supported_currencies', 'iyzico_api_key', 'iyzico_api_secret']);

            $tab->view('setting::admin.settings.tabs.pay2out', [
                'currencies' => $currencies,
            ]);
        });
    }
    ...
    {
        return tap(new Tab('balance', "Bakiye ile Ödeme"), function (Tab $tab) {
            $tab->weight(69);
            ...
            $tab->fields(['balance_enabled', 'translatable.balance_label', 'translatable.balance_description']);

            $tab->view('setting::admin.settings.tabs.balance', [
                'currencies' => $currencies,
            ]);
        });
    }

    private function shopier()
    {
        return tap(new Tab('shopier', "Shopier"), function (Tab $tab) {
            $tab->weight(69);
            ...
            $tab->view('setting::admin.settings.tabs.shopier', [
                'currencies' => $currencies,
            ]);
        });
    }
